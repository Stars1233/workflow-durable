...
def validate_plan(plan: Any) -> None:
    ...
    if set(plan) != expected or plan.get("schema") not in {LEGACY_SCHEMA, SCHEMA}:
        raise RecoveryError(
            "release plan does not satisfy a supported channel-aware contract"
        )
    ...
    channel_components = (
        COMPONENTS if plan["channel"] == "rc" else ("workflow", "waterline")
    )
    ...
    if aggregate_rc_foundation and authorization is not None:
        raise RecoveryError(
            "aggregate release-candidate plans cannot claim beta qualification"
        )
    if (
...
def verify_plan_authority(
    ...
    if foundation_identity["tag"] == FOUNDATION_TAG:
        ...
        if (
            ...
            != "durable-workflow.beta-candidate-verification/v2"
            ...
            or any(
                ...
                for result in verification_components.values()
            )
        ):
            raise RecoveryError(
                    ...
                    no_op.update(
                        {
                            ...
                            "reason": str(error),
                            "resume_action": (
                                "No action is required; scheduled recovery found "
                                "no eligible release plan"
                            ),
                        }
                    )
