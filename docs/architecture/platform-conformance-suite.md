...
published crates.io `durable-workflow` crate at exactly `2.0.0-rc.4`. The
...
- `published_artifact_install_only` — server image, CLI installer,
  ...
  `durable-workflow` with the exact Cargo requirement `=2.0.0-rc.4`.
...
- `rust_replayed_instance_state_query_after_cold_restart` — using the
  exact published crates.io `durable-workflow =2.0.0-rc.4` crate, start and
